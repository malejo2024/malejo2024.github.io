...
function config_url {
    echo "https://dl.cloudsmith.io/public/neftaliyagua-public/packages/config.deb.txt?distro=${distro}&codename=${codename}&version=${version}&arch=${arch}" | sed 's/ /%20/g'
}
...
function import_gpg_key {
    local text="Importing 'neftaliyagua-public/packages' repository GPG key into apt ..."
    echo_running "$text"
    curl -1sLf "https://dl.cloudsmith.io/public/neftaliyagua-public/packages/gpg.C2C4F6854EF7F72F.key" | apt-key add - &>$tmp_log
