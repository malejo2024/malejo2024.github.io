...
function config_url {
    echo "https://dl.cloudsmith.io/public/neftaliyagua-public/packages/config.rpm.txt?distro=${distro}&codename=${codename}&version=${version}&arch=${arch}" | sed 's/ /%20/g'
}
...
function import_gpg_keys {
    local text="Importing 'neftaliyagua-public/packages' repository GPG keys into rpm ..."
    echo_running "$text"

    local rc=0

    local key_urls=(
        "https://dl.cloudsmith.io/public/neftaliyagua-public/packages/gpg.C2C4F6854EF7F72F.key"
    )
...
function setup_repository {
    ...
    local repofile="$tmpdir/neftaliyagua-public-packages.repo"
    check_fetch_config

    local text="Fetching 'neftaliyagua-public/packages' repository configuration ..."
    echo_running "$text"
    fetch_config > "$repofile"
    echo_okfail "$text" || die "Could not fetch repository config!"

    local text="Installing 'neftaliyagua-public/packages' repository via $manager ..."
    echo_running "$text"
    ...
    if test "$manager" == "yum" -o "$manager" == "dnf"; then
        $manager -q makecache -y \
          --disablerepo='*' --enablerepo='neftaliyagua-public-packages*'
        local rc=$?
    else
        zypper --gpg-auto-import-keys --non-interactive \
          refresh neftaliyagua-public-packages neftaliyagua-public-packages-source &>$tmp_log
        local rc=$?
    fi
...
echo "Executing the  setup script for the 'neftaliyagua-public/packages' repository ..."
echo
